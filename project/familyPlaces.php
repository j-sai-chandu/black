...
// Query to fetch `family_places`, `family_best_time`, and `family_min_cost` based on `id`
$sql = "SELECT family_places, family_best_time, family_min_cost FROM destinations WHERE id = ?";
...
if ($result->num_rows > 0) {
    ...
    $familyPlaces = $row['family_places'];
    $familyBestTime = $row['family_best_time'];
    $familyMinCost = $row['family_min_cost'];

    // Convert comma-separated values into arrays
    $placesArray = !empty($familyPlaces) ? explode(',', $familyPlaces) : [];
    $bestTimeArray = !empty($familyBestTime) ? explode(',', $familyBestTime) : [];
    $minCostArray = !empty($familyMinCost) ? explode(',', $familyMinCost) : [];
...
<body>
    ...
    <div class="places-container">
        ...
        <div class="total-cost">
            Total Family Travel Expenses: €<?php echo number_format($totalCost, 2); ?> <!-- Format total cost -->
        </div>
