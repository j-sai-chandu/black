...
$email = $_POST['email'];
$password = $_POST['password']; // No hashing, plain text password
...
if ($result->num_rows > 0) {
    ...
    if ($update_stmt->execute()) {
        ...
        echo "<script>alert('Error updating password. Please try again later.'); window.location.href = 'forgot_password.html';</script>";
    }
} else {
    // If no account is found with the entered email
    echo "<script>alert('No account found with this email.'); window.location.href = 'forgot_password.html';</script>";
}
