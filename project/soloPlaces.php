...
// Query to fetch `solo_places`, `solo_best_time`, and `solo_min_cost` based on `id`
$sql = "SELECT solo_places, solo_best_time, solo_min_cost FROM destinations WHERE id = ?";
...
if ($result->num_rows > 0) {
    ...
    $soloPlaces = $row['solo_places'];
    $soloBestTime = $row['solo_best_time'];
    $soloMinCost = $row['solo_min_cost'];

    // Convert comma-separated values into arrays
    $placesArray = !empty($soloPlaces) ? explode(',', $soloPlaces) : [];
    $bestTimeArray = !empty($soloBestTime) ? explode(',', $soloBestTime) : [];
    $minCostArray = !empty($soloMinCost) ? explode(',', $soloMinCost) : [];
...
<head>
    ...
    <title>Solo Traveler RoadMap</title>
    <style>
        body {
...
<body>
    ...
    <div class="places-container">
        ...
        <div class="total-cost">
            Total Solo Travel Expenses: €<?php echo number_format($totalCost, 2); ?> <!-- Format total cost -->
        </div>
