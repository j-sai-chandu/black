...
// Query to fetch `friends_places`, `friends_best_time`, and `friends_min_cost` based on `id`
$sql = "SELECT friends_places, friends_best_time, friends_min_cost FROM destinations WHERE id = ?";
...
if ($result->num_rows > 0) {
    ...
    $friendsPlaces = $row['friends_places'];
    $friendsBestTime = $row['friends_best_time'];
    $friendsMinCost = $row['friends_min_cost'];

    // Convert comma-separated values into arrays
    $placesArray = !empty($friendsPlaces) ? explode(',', $friendsPlaces) : [];
    $bestTimeArray = !empty($friendsBestTime) ? explode(',', $friendsBestTime) : [];
    $minCostArray = !empty($friendsMinCost) ? explode(',', $friendsMinCost) : [];
...
<body>
    ...
    <div class="places-container">
        ...
        <div class="total-cost">
            Total Friends Travel Expenses: €<?php echo number_format($totalCost, 2); ?> <!-- Format total cost -->
        </div>
